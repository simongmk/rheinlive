...
test('dated map networks contain real region stops and separate transport-mode keys',async()=>{for(const city of Object.values(cities)){const d=JSON.parse(await readFile(new URL('../public/data/network-'+city.id+'.json',import.meta.url)));assert.equal(d.kind,'static-network-not-live-vehicles');assert.equal(d.city,city.id);assert.ok(d.catalog.some(l=>l.mode==='bus'));assert.ok(d.catalog.some(l=>l.mode==='tram'));assert.ok(d.lines.features.length>100);assert.ok(d.stops.features.every(f=>pointInBounds([f.geometry.coordinates[1],f.geometry.coordinates[0]],city.bounds)));assert.ok(d.catalog.every(l=>l.key===lineKey(l.mode,l.line)));}});
