...
test('excludes non-Stadtbahn modes, foreign lines, invalid coordinates and cancelled trips',()=>{assert.equal(normaliseSegments([segment({mode:'BUS'}),segment({trips:[{tripId:'x',routeShortName:'999'}]}),segment({from:place('A',NaN,7)}),segment({cancelled:true}),segment({from:place('Hamburg',53.5,10),to:place('Hamburg2',53.6,10)})]).trips.length,0);});
