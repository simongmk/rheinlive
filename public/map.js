...
export async function createTransitMap(city,{onSelect=()=>{},onMove=()=>{},onClear=()=>{},onError=()=>{}}={}){
  ...
  function mount(){
    ...
    map.addLayer({id:'vehicle-dots',type:'circle',source:'vehicles',paint:{'circle-radius':['interpolate',['linear'],['zoom'],9,10,12,15,16,17],'circle-color':['case',['get','planned'],theme==='dark'?'#142838':'#ffffff',['get','color']],'circle-stroke-color':['case',['get','planned'],'#8c9eab','#ffffff'],'circle-stroke-width':['case',['get','planned'],1,1.8],'circle-opacity':['case',['get','planned'],.65,1]}});
    map.addLayer({id:'vehicle-labels',type:'symbol',source:'vehicles',layout:{'text-field':['get','label'],'text-font':['Noto Sans Bold'],'text-size':['interpolate',['linear'],['zoom'],9,9,12,11,16,12],'text-allow-overlap':true,'text-ignore-placement':true},paint:{'text-color':['case',['get','planned'],theme==='dark'?'#a8b9c5':'#475a66',['get','textColor']]}});
    applyFilter();render();
  }
  function render(){if(!map.getSource('vehicles'))return;map.getSource('vehicles').setData(collection(vehicles.map(v=>point([v.lon,v.lat],{id:v.id,label:v.line,color:v.color,textColor:v.textColor||'#ffffff',planned:v.quality==='schedule'}))));map.setFilter('vehicle-selection',['==',['get','id'],selected||'']);}
  ...
  return {
    ...
    setFilter:(modes,lineKeys)=>{filter=['all',['in',['get','mode'],['literal',modes]],...(lineKeys?[['in',['get','lineKey'],['literal',lineKeys]]]:[])];applyFilter();},
