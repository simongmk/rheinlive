...
const headers={
  ...
  'Content-Security-Policy':"default-src 'self'; script-src 'self' https://unpkg.com; style-src 'self' 'unsafe-inline' https://unpkg.com; img-src 'self' data: https://tile.openstreetmap.org; connect-src 'self'; font-src 'self'; object-src 'none'; base-uri 'self'; form-action 'none'; frame-ancestors 'self' https://chatgpt.com https://chat.openai.com",
};
...
export function createWorker({api=handleApi,clock=Date.now,getEdgeCache=()=>globalThis.caches?.default}={}) {
  return {
    async fetch(request,env,ctx){
      ...
      const cacheKey=new Request(new URL('/api/vehicles?city=cologne',url.origin));
      const cacheable=url.pathname==='/api/vehicles'&&request.method==='GET'&&(!url.searchParams.has('city')||url.searchParams.get('city')==='cologne');
