...
export function departureReadiness(event,{now,fetchedAt,validUntil,walkMinutes,bufferMinutes=2}){
  if(!Number.isFinite(now)||!Number.isFinite(fetchedAt)||now-fetchedAt>MAX_SNAPSHOT_AGE_MS||fetchedAt-now>30000||(validUntil!==undefined&&(!Number.isFinite(validUntil)||now>validUntil)))return {state:'stale'};
  ...
  return {state:leaveAt<now?'tight':seconds<=60?'leave':'ready',leaveAt,seconds,departureSeconds:Math.ceil((event.departure-now)/1000)};
}
/** Only this station's recorded lines, plus lines actually returned on its board. */
export function stationLineOptions(station,departures=[],catalog=[]){
  const known=new Map(catalog.map(e=>[e.key,e])),lines=new Map();
  for(const key of station?.properties.lineKeys??[]){const split=key.indexOf(':'),mode=key.slice(0,split),line=key.slice(split+1),entry=known.get(key);if(split<1||!line)continue;lines.set(key,{key,mode,line,color:entry?.color??lineColor(mode,line)});}
  for(const e of departures)lines.set(e.lineKey,{key:e.lineKey,line:e.line,mode:e.mode,color:e.color});
  return [...lines.values()].sort((a,b)=>a.line.localeCompare(b.line,'de',{numeric:true})||a.mode.localeCompare(b.mode));
}
export function selectMonitorDepartures(departures,options){
  const {now,lines=null,direction=''}=options;
  const upcoming=departures.filter(e=>e.departure>now&&(lines===null||lines.has(e.lineKey))&&(!direction||e.directionKey===direction)).sort((a,b)=>a.departure-b.departure);
  const primary=[],directions=new Set();
  for(const e of upcoming){const r=departureReadiness(e,options),group=e.lineKey+'|'+e.directionKey;if(['ready','leave','tight'].includes(r.state)&&!directions.has(group)){primary.push(e);directions.add(group);if(primary.length===2)break;}}
