...
export function groupStations(features){
  ...
  for(const f of features){
    ...
    if(!g){g={parent,name,id:parent||String(p.id||name+':'+xy.join(',')),queryId:p.id||null,anchor:xy,points:new Map(),modes:new Set(),lineKeys:new Set()};groups.push(g);if(!names.has(key))names.set(key,[]);names.get(key).push(g);}
