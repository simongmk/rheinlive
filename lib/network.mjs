...
export function groupStations(features){
  ...
  for(const f of features){
    const p=f.properties,xy=f.geometry.coordinates,name=p.name.trim(),key=name.toLocaleLowerCase('de'),parent=p.sourceParentId||parentId(p.id);
    ...
    if(!g){g={parent,name,id:parent||String(p.id||name+':'+xy.join(',')),queryId:p.queryId||p.id||null,anchor:xy,points:new Map(),modes:new Set(),lineKeys:new Set()};groups.push(g);if(!names.has(key))names.set(key,[]);names.get(key).push(g);}
  ...
  return collection(groups.map(g=>{const ps=[...g.points.values()],mean=ps.reduce((a,p)=>[a[0]+p[0]/ps.length,a[1]+p[1]/ps.length],[0,0]);return {type:'Feature',properties:{id:g.id,queryId:g.queryId,name:g.name,modes:[...g.modes].sort(),lineKeys:[...g.lineKeys].sort()},geometry:{type:'Point',coordinates:mean.map(n=>+n.toFixed(5))}};}));
}
