...
function stop(p,platformResolver){
  if(!p||!validPoint([p.lat,p.lon])||!p.name)return null;
  return {id:clean(p.stopId,1000),name:clean(p.name),lat:p.lat,lon:p.lon,arrival:time(p.arrival),departure:time(p.departure),scheduledArrival:time(p.scheduledArrival),scheduledDeparture:time(p.scheduledDeparture),...platformResolver(p),cancelled:p.cancelled===true,pickupType:clean(p.pickupType,30),dropoffType:clean(p.dropoffType,30)};
}
export function normaliseTripDetail(raw,platformResolver=platformFields){
  ...
  for(const leg of legs.slice(0,10)){
    for(const p of [leg.from,...(leg.intermediateStops||[]).slice(0,250),leg.to]){const s=stop(p,platformResolver);if(!s)continue;const previous=stops.at(-1);if(previous?.id===s.id&&previous?.arrival===s.arrival)continue;stops.push(s);}
