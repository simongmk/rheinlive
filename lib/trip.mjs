...
function stop(p){
  ...
  return {id:clean(p.stopId,1000),name:clean(p.name),lat:p.lat,lon:p.lon,arrival:time(p.arrival),departure:time(p.departure),scheduledArrival:time(p.scheduledArrival),scheduledDeparture:time(p.scheduledDeparture),track:clean(p.track,30),scheduledTrack:clean(p.scheduledTrack,30),cancelled:p.cancelled===true,pickupType:clean(p.pickupType,30),dropoffType:clean(p.dropoffType,30)};
}
