...
export function createMonitorApi({fetcher=fetch,clock=Date.now,readJSON}){
  ...
  async function board(city,id){
    ...
    const task=queue.then(async()=>{
      ...
      let data;try{const raw=await source(url);data={city:city.id,stopId:id,fetchedAt:clock(),stale:false,departures:normaliseDepartures(raw,city,stationIndex[city.id][id])};}
