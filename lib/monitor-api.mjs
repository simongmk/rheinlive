...
export function createMonitorApi({fetcher=fetch,clock=Date.now,readJSON,baseUrl='https://api.transitous.org',sourceName='Transitous / MOTIS',stations=stationIndex,platformResolver=platformFields}){
  ...
  async function board(city,id){
    ...
    const task=queue.then(async()=>{
      const url=new URL('/api/v6/stoptimes',baseUrl);url.search=new URLSearchParams({stopId:id,n:'60',arriveBy:'false',language:'de',withAlerts:'false'});
      let data;try{const raw=await source(url);const departures=normaliseDepartures(raw,city,stations[city.id][id]);for(const e of departures)e.stop={...e.stop,...platformResolver(e.stop)};data={city:city.id,stopId:id,fetchedAt:clock(),stale:false,departures,source:sourceName};}
  ...
  return async request=>{
    ...
    if(url.pathname==='/api/departures'){
      ...
      if(!city||!Object.hasOwn(stations[city.id]??{},id))return json({error:'Haltestelle in dieser Region nicht verfügbar.'},404);
    ...
    if(url.pathname==='/api/walk'){
      ...
      if(!city||!validPoint(origin)||!pointInBounds(origin,city.bounds)||!Array.isArray(ids)||!ids.length||ids.length>6||new Set(ids).size!==ids.length||ids.some(id=>!Object.hasOwn(stations[city.id]??{},id)||distance(origin,stations[city.id][id])>4000))return json({error:'Fußwege nur zu nahen Haltestellen der Region verfügbar.'},400);
      if(walkPending>=3)return json({error:'Gehzeitberechnung gerade ausgelastet.'},429);
      walkPending++;
      try{
        const u=new URL('/api/v1/one-to-many',baseUrl);u.search=new URLSearchParams({one:origin.join(';'),many:ids.map(id=>stations[city.id][id].join(';')).join(','),mode:'WALK',max:'3600',maxMatchingDistance:'100',arriveBy:'false',withDistance:'true'});
        ...
        return json({city:city.id,fetchedAt:clock(),walks,source:sourceName+' · Fußwegenetz',estimated:true});
