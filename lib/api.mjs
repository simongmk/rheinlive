...
import {normaliseSegments,retainRecentArrivals} from './transit.mjs';
import {createMonitorApi} from './monitor-api.mjs';
import {platformFields} from './platforms.mjs';
import {stationIndex as defaultStations} from './station-index.mjs';
const BASE='https://api.transitous.org';
const SOURCE='Transitous / MOTIS';
const CACHE_MS=30_000, RETRY_MS=30_000, FETCH_TIMEOUT_MS=15_000, MAX_RESPONSE_BYTES=12_000_000;
export function upstreamUrl(now,configuredCity=city,baseUrl=BASE) {
  const minute=Math.floor(now/60_000)*60_000;
  const u=new URL('/api/v6/map/trips',baseUrl);
...
export function createTransitService({fetcher=fetch,clock=Date.now,baseUrl=BASE,sourceName=SOURCE,platformResolver=platformFields}={}) {
  ...
  return async function snapshot(cityId='cologne'){
    ...
    function failure(time){return {city:cityId,fetchedAt:state.cache?.fetchedAt??null,serverTime:time,stale:true,source:sourceName,trips:[],error:'Verkehrsdaten momentan nicht erreichbar. Alte Positionen werden ausgeblendet.'};}
    if(now-state.lastFailure<RETRY_MS)return failure(now);
    if(state.inflight)return state.inflight;
    state.inflight=queue.then(async()=>{try{
      const response=await fetcher(upstreamUrl(clock(),configuredCity,baseUrl),{headers:{'Accept':'application/json','User-Agent':'Rheinlive/0.2 (https://github.com/simongmk/rheinlive/issues)'},signal:AbortSignal.timeout(FETCH_TIMEOUT_MS)});
      const raw=await readBoundedJSON(response),{trips,rejected}=normaliseSegments(raw,configuredCity);
      for(const trip of trips)for(const s of trip.segments)for(const side of ['from','to'])s[side]={...s[side],...platformResolver(s[side])};
      const receivedAt=clock();
      const invalidatedIds=new Set(raw.filter(s=>s?.cancelled===true).flatMap(s=>Array.isArray(s.trips)?s.trips.map(t=>t.tripId):[]));
      state.cache={city:cityId,fetchedAt:receivedAt,serverTime:receivedAt,stale:false,source:sourceName,trips:retainRecentArrivals(trips,state.cache,receivedAt,invalidatedIds),rejected};return state.cache;
...
/** All four APIs and their caches belong to one configured provider and station namespace. */
export function createApi({fetcher=fetch,clock=Date.now,baseUrl=BASE,sourceName=SOURCE,stations=defaultStations,assertFresh=async()=>{},platformResolver=platformFields}={}){
const getSnapshot=createTransitService({fetcher,clock,baseUrl,sourceName,platformResolver});
const details=new Map(),detailInflight=new Map();let detailQueue=Promise.resolve();
async function getDetail(id){
  const previous=details.get(id);if(previous&&clock()-previous.fetchedAt<30_000)return previous;
  ...
  const result=detailQueue.then(async()=>{
    const u=new URL('/api/v6/trip',baseUrl);u.search=new URLSearchParams({tripId:id,language:'de',detailedLegs:'true'});
    const r=await fetcher(u,{headers:{'User-Agent':'Rheinlive/0.2 (https://github.com/simongmk/rheinlive/issues)'},signal:AbortSignal.timeout(FETCH_TIMEOUT_MS)});
    const data=normaliseTripDetail(await readBoundedJSON(r,2_000_000),platformResolver);
    const detail={...data,fetchedAt:clock(),positionType:'estimated',source:sourceName};
...
const monitorApi=createMonitorApi({fetcher,clock,readJSON:readBoundedJSON,baseUrl,sourceName,stations,platformResolver});
return async function handleApi(request){
  const u=new URL(request.url);if(!u.pathname.startsWith('/api/'))return null;
  if(['/api/vehicles','/api/trip','/api/departures'].includes(u.pathname)){
    try{await assertFresh();}catch{return Response.json({city:u.searchParams.get('city')||'cologne',stale:true,trips:[],departures:[],source:sourceName,serverTime:clock(),error:'Aktuelle Datenquelle momentan nicht verfügbar.'},{status:503,headers:{'Cache-Control':'no-store'}});}
  }
  if(['/api/departures','/api/walk'].includes(u.pathname))return monitorApi(request);
  ...
  if(u.pathname==='/api/vehicles'){
    const data=await getSnapshot(cityId);return Response.json({...data,serverTime:clock()},{status:data.stale?503:200,headers:{'Cache-Control':'no-store','X-Content-Type-Options':'nosniff'}});
  }
