...
export function createTransitService({fetcher=fetch,clock=Date.now}={}) {
  ...
  return async function snapshot(cityId='cologne'){
    ...
    state.inflight=queue.then(async()=>{try{
      ...
      for(const trip of trips)for(const s of trip.segments)for(const side of ['from','to'])s[side]={...s[side],...platformFields(s[side])};
      const receivedAt=clock();
      const invalidatedIds=new Set(raw.filter(s=>s?.cancelled===true).flatMap(s=>Array.isArray(s.trips)?s.trips.map(t=>t.tripId):[]));
      state.cache={city:cityId,fetchedAt:receivedAt,serverTime:receivedAt,stale:false,source:'Transitous / MOTIS',trips:retainRecentArrivals(trips,state.cache,receivedAt,invalidatedIds),rejected};return state.cache;
...
const monitorApi=createMonitorApi({readJSON:readBoundedJSON});
