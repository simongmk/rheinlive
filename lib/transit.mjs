...
export function prepareTrips(trips,{previous=[],now,previousFetchedAt}={}) {
  const previousById=new Map(previous.map(t=>[t.id,t]));
  return trips.map(t=>{
    const segments=t.segments.map(prepareSegment),dwells=segments.map((s,i)=>dwellBetween(s,segments[i+1],t.mode));
    const old=previousById.get(t.id),moving=old&&positionAt(old,now,previousFetchedAt);
    if(moving?.state==='moving'&&old.mode===t.mode){
      const before=moving.segment,oldDwell=old.dwells?.[old.segments.indexOf(before)];
      for(let i=0;i<segments.length;i++){
        const s=segments[i];
        // A rolling map window may reveal the outgoing leg only mid-journey.
        // Do not retroactively shorten an unchanged journey to insert a modeled
        // stop: that teleports every affected icon on the same polling frame.
        // Real prediction changes and reported stays still apply immediately.
        if(dwells[i]?.kind==='estimated'&&!oldDwell&&s.from.id===before.from.id&&s.to.id===before.to.id&&s.departure===before.departure&&s.arrival===before.arrival&&s.realtime===before.realtime)dwells[i]=null;
      }
    }
    return {...t,segments,dwells};
  });
}
