...
function stop(raw) {
  ...
  return {id:String(raw.stopId??raw.name),name:raw.name.slice(0,200),lat:raw.lat,lon:raw.lon,track:typeof raw.track==='string'?raw.track.slice(0,30):null};
}
...
export function positionAt(trip, now, fetchedAt) {
  ...
  for(let i=0;i<trip.segments.length;i++) {
    ...
    if(next&&freshSegment(next,now,fetchedAt)&&s.to.id===next.from.id&&now>=s.arrival&&now<next.departure&&next.departure>s.arrival&&next.departure-s.arrival<=3_600_000) {
      const quality=s.realtime&&next.realtime?'realtime':'schedule';
      return {lat:next.from.lat,lon:next.from.lon,bearing:0,segment:next,state:'stopped',quality,fraction:0,dwell:{arrival:s.arrival,departure:next.departure,durationMs:next.departure-s.arrival,quality}};
    }
